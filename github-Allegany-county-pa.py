...
exit_app = False
ALLEGANY_FIPS = "42003"  # Allegany County FIPS code
        ...
        if data['features']:
            ...
            for alert_feature in data['features']:
                ...
                if ALLEGANY_FIPS in fips_list:
                    ...
                    print(f"[{datetime.now()}] Skipped alert not for Allegany County (FIPS filter).")
        else:
...
def create_icon():
    ...
    draw.ellipse((16, 16, 48, 48), fill=(255, 215, 0))  # gold circle
    return image

def setup_tray():
    icon_image = create_icon()
    menu = Menu(MenuItem('Quit', quit_app))
    tray_icon = Icon("Weather Alert", icon_image, "Allegany Alerts", menu)
    tray_icon.run()
