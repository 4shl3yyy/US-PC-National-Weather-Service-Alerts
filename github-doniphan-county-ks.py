...
exit_app = False
DONIPHAN_FIPS = "20043"  # Doniphan County, Kansas FIPS code
        ...
        if data['features']:
            ...
            for alert_feature in data['features']:
                ...
                if DONIPHAN_FIPS in fips_list:
                    ...
                    print(f"[{datetime.now()}] Skipped alert not for Doniphan County (FIPS filter).")
        else:
...
def create_icon():
    ...
    draw.ellipse((16, 16, 48, 48), fill=(0, 128, 255))  # Blue circle
    return image

def setup_tray():
    icon_image = create_icon()
    menu = Menu(MenuItem('Quit', quit_app))
    tray_icon = Icon("Weather Alert", icon_image, "Doniphan Alerts", menu)
    tray_icon.run()
