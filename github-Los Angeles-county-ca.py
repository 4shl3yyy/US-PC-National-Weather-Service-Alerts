...
exit_app = False
LOS_ANGELES_FIPS = "06037"  # Los Angeles County, CA FIPS code
    ...
    try:
        ...
        if data['features']:
            ...
            for alert_feature in data['features']:
                ...
                if LOS_ANGELES_FIPS in fips_list:
                    ...
                    print(f"[{datetime.now()}] Skipped alert not for Los Angeles County (FIPS filter).")
        else:
...
def create_icon():
    ...
    draw.ellipse((16, 16, 48, 48), fill=(255, 0, 0))  # red circle for LA
    return image

def setup_tray():
    icon_image = create_icon()
    menu = Menu(MenuItem('Quit', quit_app))
    tray_icon = Icon("Weather Alert", icon_image, "LA Alerts", menu)
    tray_icon.run()
