...
exit_app = False
PHILADELPHIA_FIPS = "42101"  # Philadelphia County FIPS code
    ...
    try:
        ...
        if data['features']:
            ...
            for alert_feature in data['features']:
                ...
                if PHILADELPHIA_FIPS in fips_list:
                    ...
                    print(f"[{datetime.now()}] Skipped alert not for Philadelphia County (FIPS filter).")
        else:
...
def setup_tray():
    ...
    tray_icon = Icon("Weather Alert", icon_image, "Philadelphia Alerts", menu)
    tray_icon.run()
