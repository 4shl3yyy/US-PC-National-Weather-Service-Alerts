        ...
        if data['features']:
            ...
            for alert_feature in data['features']:
                ...
                if LUZERNE_FIPS in fips_list:
                    ...
                    print(f"[{datetime.now()}] Skipped alert not for Luzerne County (FIPS filter).")
        else:
...
def setup_tray():
    ...
    tray_icon = Icon("Weather Alert", icon_image, "Luzerne Alerts", menu)
    tray_icon.run()
